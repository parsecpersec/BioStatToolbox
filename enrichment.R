...
Deg <- read.csv('./geo.csv', T, stringsAsFactors=F)

setup <- Deg$gene[Deg$Pvalue < 0.05 & Deg$log2_FC > 0]

GO <- GOKEGG(geneset=setup, output='GO', filename='geo_up', trend='UP')
KEGG <- GOKEGG(geneset=setup, output='KEGG', filename='geo_up', trend='UP')
